...
export const generatePdfForCurrentUser =
  () => async (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    if (!templates.length) { console.error("Templates not loaded"); return; }
    ...
    if (!cover || cover.templateId !== tpl._id) {          // <- свойство зависит от DTO
    ...
    /* фото */
    const photos = selectPhotos(state).map(p => ({
      imgData           : undefined as any,
      photoUrl          : p.photoUrl,
      date              : typeof p.date === "string"
                           ? p.date
